...
class GameStateNotifier extends _$GameStateNotifier {
  ...
  void _watchMatch(String matchId) {
    _matchSubscription = _matchRepository.watchMatch(matchId).listen(
      (match) {
        if (match == null) {
  ...
  Future<void> _startShootingCountdown(int roundNumber) async {
    ...
    // カメラ起動
    try {
      await _cameraService.startCameraWithCountdown(countdownSeconds: 5);
    } catch (e) {
  ...
  Future<void> _onShootingCountdownComplete(int roundNumber) async {
    ...
    await _matchRepository.updateRound(
  ...
  Future<void> _uploadCapturedImage(int roundNumber) async {
    ...
    try {
      // ネイティブから撮影画像のパスを取得
      final imagePath = await _cameraService.getCapturedImagePath();
      if (imagePath == null) {
      ...
      final playerNumber =
          currentState.isPlayer1(_currentUserId) ? 1 : 2;

      // Firebase Storageにアップロード
      final uploadStartTime = DateTime.now();
      final imageUrl = await _storageService.uploadMatchImage(
        matchId: currentState.matchId,
        roundNumber: roundNumber,
      ...
      await _matchRepository.updateImageUpload(
        matchId: currentState.matchId,
        roundNumber: roundNumber,
      ...
      // アップロード時間を記録
      final isPlayer1 = currentState.isPlayer1(_currentUserId);
      await _matchRepository.updateRound(
        matchId: currentState.matchId,
        roundNumber: roundNumber,
        roundData: {
  ...
  Future<void> _startDisplayCountdown(GamePhase phase) async {
    ...
    if (roundData != null) {
      final isPlayer1 = currentState.isPlayer1(_currentUserId);
      final opponentImageUrl =
          isPlayer1 ? roundData.player2ImageUrl : roundData.player1ImageUrl;
  ...
  Future<void> _startPlaying(int roundNumber) async {
    ...
    // 対戦中フェーズへ
    final playingPhase =
        roundNumber == 1 ? GamePhase.round1Playing : GamePhase.round2Playing;
    state = currentState.copyWith(
      phase: playingPhase,
    ...
    // 笑顔判定結果を監視
    _smileSubscription?.cancel();
    _smileSubscription = _cameraService.smileDetectionStream.listen(
      (result) {
        final current = state;
  ...
  Future<void> _onPlayerSmiled(int roundNumber) async {
    ...
    _playingTimer?.cancel();
    _smileSubscription?.cancel();
    await _cameraService.stopSmileDetection();

    // 笑顔検知時刻を記録
    await _matchRepository.updateSmileDetected(
      matchId: currentState.matchId,
      roundNumber: roundNumber,
      playerId: _currentUserId,
    );
  ...
  Future<void> _onPlayingTimeout(int roundNumber) async {
    ...
    if (roundNumber == 1) {
      ...
      await _matchRepository.proceedToNextRound(currentState.matchId);
